...
public class FormDiarioCon extends javax.swing.JFrame {
    ...
    private void atualizaPeriodos() {
        ...
        data.access.object.DAOPeriodo DAOPeriodo = new data.access.object.DAOPeriodo();

        try {

            List periodos = DAOPeriodo.getPeriodos();
            for (int i = 0; i <= periodos.size() - 1; i++) {

                Map obj = (Map) periodos.get(i);
                this.periodoCmb.addItem(obj.get("PERIODO"));
    ...
    private void atualizaOcorrencias() {
        ...
        try {
            ...
            List ocorrencias = DAOOcorrencia.getOcorrencias();
    ...
    private void atualizaTecnicosPorSetor() {
        ...
        try {
            List tecnicos = DAOTecnico.getTecnicosPorSetor(setores);
            for (int i = 0; i <= tecnicos.size() - 1; i++) {

                Map obj = (Map) tecnicos.get(i);
                this.tecnicoCmb.addItem(obj.get("TECNICO"));
    ...
    private void atualizaTecnicosPorUF() {
        ...
        try {
            ...
            List tecnicos = DAOTecnico.getTecnicosPorUF(this.getUf());
            for (int i = 0; i <= tecnicos.size() - 1; i++) {
    ...
    private void atualizaDiario() throws SQLException, ClassNotFoundException {
        ...
        for (int i = 0; i <= ocorrencia.size() - 1; i++) {
